...
describe('Scheduled Backup (via Nitro Task)', () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(purgeModule, 'purgeOrphanedMediaFiles').mockResolvedValue(undefined)
    ...
    vi.spyOn(prefsRepo, 'updatePreferencesService').mockResolvedValue(makePrefsStub() as never)
  ...
  it('creates a backup file when lastBackupAt is null', async () => {
    vi.spyOn(prefsRepo, 'getPreferencesService').mockResolvedValue(makePrefsStub() as never)
  ...
  it('persists lastBackupAt to DB after a successful backup', async () => {
    vi.spyOn(prefsRepo, 'getPreferencesService').mockResolvedValue(makePrefsStub() as never)
    const updateSpy = vi.spyOn(prefsRepo, 'updatePreferencesService').mockResolvedValue(makePrefsStub() as never)

    await runJobs()

    expect(updateSpy).toHaveBeenCalledWith(
      expect.objectContaining({ lastBackupAt: expect.any(Date) }),
    )
  })
})
