...
### Purpose
Answers: *"How consistently has this person shown up for this identity over the last N days?"*
...
`toISOString()` converts to UTC. A user in `UTC+2` at 11 PM would see the *previous* calendar day as today — causing completions logged at local midnight to be invisible until the next UTC rollover.
...
Using total days punishes the user for following their own schedule. A `weekdays` habit completed perfectly every Monday–Friday would score only **71%** against a 7-day denominator. Using only the days the habit was *due*, 100% means "I showed up every time I said I would."
...
### Purpose
Answers: *"How many votes has this person cast for their identity, and how committed were each of those votes?"*
...
| *(unknown)* | 0 | Silently ignored. Cannot occur under DB enum constraint. |
...
- **Lifetime, not windowed.** Votes accumulate forever. This is a measure of *total evidence* gathered for an identity. It grows monotonically and never decreases.
