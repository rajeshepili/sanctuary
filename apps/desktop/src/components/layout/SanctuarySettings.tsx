          ...
          {(
            [
              ...
              [
                'immersive',
                'Full-screen scene',
                'Scroll down from the landscape.',
              ],
            ] as const
          ...
          <Tooltip>
            ...
            <TooltipContent side="top" className="max-w-sm p-3 text-xs">
              <div className="flex flex-col gap-2.5">
                <p>
                  <strong>Why:</strong> Calculates accurate sunrise and sunset
                  times to dynamically change the background scene to match your
                  local time of day.
                </p>
                <p>
                  <strong>What:</strong> Only rough latitude and longitude are
                  collected.
                </p>
                <p>
                  <strong>How often:</strong> Only fetched exactly once when you
                  click the button.
                </p>
                <p>
                  <strong>Tip:</strong> Once granted and saved, you can
                  completely disable location services in your OS/browser and
                  Sanctuary will continue using the saved coordinates
                  permanently.
                </p>
              </div>
            </TooltipContent>
...
export function SanctuarySettings() {
  ...
  const [passphraseModal, setPassphraseModal] = useState<
    'export-setup' | 'export-run' | 'restore' | null
  >(null)
  ...
  const handleRestoreJson = async (mode: 'merge' | 'replace') => {
    if (!window.sanctuary?.selectBackupFile || !window.sanctuary.readFileText)
      return
    ...
    try {
      ...
      invalidateJournal()
      toast.success(
        `Restored ${result.entriesImported} entries and ${result.identitiesImported} identities.`,
      )
    } catch (e) {
  ...
  const handleOpenBackupFolder = async () => {
    ...
    const dir =
      backupStatus?.backupPath ?? (await window.sanctuary.getBackupDir())
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="w-[92vw] sm:max-w-4xl p-0 gap-0 h-[min(85vh,720px)] flex flex-col">
          <DialogHeader className="px-6 py-5 border-b shrink-0">
            ...
            <DialogDescription>
              <a
                href={`${APP_REPO_URL}/blob/main/docs/USER_GUIDE.md`}
                target="_blank"
                rel="noreferrer"
                ...
                {activeTab === 'data' && (
                  <div className="space-y-8">
                    {isDesktop && (
                      <section className="space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Saves a JSON file to your computer while Sanctuary is
                          open. Not a cloud service.
                        </p>
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            <RefreshCw
                              className={`w-4 h-4 mr-1 ${backupLoading ? 'animate-spin' : ''}`}
                            />
                            Back up now
                          </Button>
                        </div>
                    ...
                    <section className="space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Pick a `.json` or `.enc` backup exported from Sanctuary.
                      </p>
                      <div className="flex gap-2">
                        <Button
                          type="button"
                          ...
                          onClick={() => void handleRestoreJson('merge')}
                        >
                          <Upload className="w-4 h-4 mr-1" /> Merge data
                        </Button>
                        <Button
                          type="button"
                          variant="destructive"
                          size="sm"
                          disabled={restoreLoading}
                          onClick={() => void handleRestoreJson('replace')}
                        >
                          Replace all data
                        </Button>
                      </div>
                    ...
                    <section className="space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        JSON backups include entries, image metadata,
                        identities, categories, and safe preferences in plain
                        text. Image files themselves stay in your local media
                        folder and are not embedded in JSON or encrypted
                        backups.
                      </p>
                      <div className="grid gap-2">
                        ...
                        >
                          <Download className="w-4 h-4 mr-2" /> Single Markdown
                          file
                        </Button>
                        <Button
                          type="button"
                ...
                {activeTab === 'security' && (
                  <div className="space-y-6">
                    <p className="text-sm text-muted-foreground">
                      PIN locks Sanctuary when you open the app. It is not
                    ...
                    <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm flex gap-3">
                      ...
                      <p className="text-muted-foreground">
                        Data stays on your device. Sanctuary is not medical or
