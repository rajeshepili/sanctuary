...
const BACKUP_FREQUENCIES = [
  ...
  { value: 'manual' as const, label: 'Manual', desc: "I'll back up from Settings." },
]
...
export function OnboardingFlow() {
  ...
  const [layoutMode, setLayoutMode] = useState<'standard' | 'immersive'>('standard')
  const [backupFrequency, setBackupFrequency] = useState<'daily' | 'weekly' | 'manual'>('weekly')
  const [backupPath, setBackupPath] = useState<string | null>(null)
  const [defaultBackupDir, setDefaultBackupDir] = useState('~/Documents/Sanctuary Backups')
    ...
    async (patch: UpdatePreferencesInput = {}) => {
      try {
        await updatePreferences.mutateAsync({
          ...
          backupEnabled: path === 'quick' ? false : backupFrequency !== 'manual',
      ...
      <Card className="w-full max-w-lg p-8 space-y-6 rounded-3xl">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              JSON copies saved to a folder on your computer while the app is running.
            </p>
            <div className="space-y-2">
