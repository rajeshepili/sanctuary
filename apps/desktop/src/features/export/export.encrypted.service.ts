import { getExportData, importBackupFromJson } from '#/features/export/export.api'
...
async function pruneOldExports(directory: string) {
  ...
  const exports = files
    .filter(
      (f) =>
        f.startsWith(ENCRYPTED_EXPORT_PREFIX) && f.endsWith(ENCRYPTED_EXPORT_EXT),
    )
    .sort()

  const stale = exports.slice(0, Math.max(0, exports.length - MAX_ENCRYPTED_EXPORTS))
  await Promise.all(
    stale.map((file) => window.sanctuary!.deleteFile!(`${directory}/${file}`)),
  )
}
...
/** Decrypts an export file and restores journal entries. */
