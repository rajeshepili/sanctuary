...
function getGridLayout(count: number) {
  ...
  if (count === 2)
    return { cols: 'grid-cols-2', heights: ['aspect-square', 'aspect-square'] }
  if (count === 3)
    return {
      cols: 'grid-cols-2',
      heights: ['aspect-square row-span-2', 'aspect-[2/1.5]', 'aspect-[2/1.5]'],
    }
  // 4+
  return {
    cols: 'grid-cols-2',
...
export const MediaGrid = memo(function MediaGridComponent({
  ...
  const overflow =
    media.length > MAX_VISIBLE ? media.length - MAX_VISIBLE + 1 : 0
  const { cols, heights } = getGridLayout(Math.min(media.length, MAX_VISIBLE))

  return (
    <>
      <div
        className={`pt-4 border-t border-border/30 mt-4 max-w-md ${className}`}
      >
