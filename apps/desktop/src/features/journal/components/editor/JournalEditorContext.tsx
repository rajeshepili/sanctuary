...
const JournalEditorContext = createContext<JournalEditorContextValue | null>(null)

export function useJournalEditorContext() {
  const context = useContext(JournalEditorContext)
  if (!context) {
    throw new Error('useJournalEditorContext must be used within a JournalEditorProvider')
  }
  return context
}
...
export function JournalEditorProvider({ children, value }: JournalEditorProviderProps) {
  return (
