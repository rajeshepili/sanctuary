...
export function CategoryManager() {
  ...
  const createForm = useForm({
    ...
    onSubmit: ({ value }) => {
      ...
      createCategory.mutate({ name: value.name.trim() })
      createForm.reset()
    },
  })

  // ── Edit helpers (inline, not a separate form instance) ───────────────────
  ...
  const handleDelete = (id: number) => {
    if (confirm('Delete this category? Identities using it will become uncategorized.'))
    ...
    <div className="space-y-4">
      ...
      >
        <createForm.Field
          ...
          validators={{
            onSubmit: ({ value }) =>
              !value.trim() ? 'Category name is required' : undefined,
          }}
          children={(field) => (
