...
export async function create(data: CreateCategoryInput): Promise<HabitCategoryEntity> {
  const db = await getDb()
  ...
  return getFirstOrThrow(results, new CategoryError('CATEGORY_CREATE_FAILED', 'Failed to create category'))
}

export async function update(data: UpdateCategoryInput): Promise<HabitCategoryEntity> {
  const db = await getDb()
  const results = await db
  ...
  return getFirstOrThrow(results, new CategoryError('CATEGORY_UPDATE_FAILED', 'Failed to update category'))
}
...
export async function remove(data: DeleteCategoryInput): Promise<void> {
  ...
  ensureRowsAffected(results, new CategoryError('CATEGORY_DELETE_FAILED', 'Failed to delete category'))
}
