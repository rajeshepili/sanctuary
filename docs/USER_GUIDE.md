...
- Restore imports entries, entry image metadata, identities, identity categories, and safe preferences (name/layout/location/backup settings)
- Backup and encrypted restore do **not** embed media binaries; keep your local `media/` folder backed up separately
...
- **JSON backup** — full snapshot (entries, image metadata, identities, categories, and safe preferences), good for restore
...
**Settings → Encrypted exports** lets you save passphrase-protected `.enc` files to a folder you choose (USB drive, Dropbox folder, etc.). You run exports manually — nothing uploads automatically. Encrypted restore follows the same scope as JSON backup restore.
