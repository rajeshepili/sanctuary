...
- Journal entries, identities, identity categories, preferences, and prompt usage
...
- **`pnpm dev:renderer`**: uses `dev.db` in the project folder unless you point `DATABASE_URL` somewhere else.
...
- JSON and encrypted backups include image metadata/paths, not image binaries.
